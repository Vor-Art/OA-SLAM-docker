    ...
    mbShutDown(false), mShouldQuit(false)
{
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     ...
                                     settings_ ? settings_->fastImuInit() : false, strSequence);
    ...
    //Initialize the Local Object Mapping thread and launch
    local_object_mapper_ = nullptr;
    if (use_objects_in_local_BA <= 1) {
        local_object_mapper_ = new LocalObjectMapping(mpAtlas->GetCurrentMap(), mpTracker);
                                    ...
                                    const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed, timestamp, filename, detections, force_relocalize);
...
void System::Shutdown()
{
    ...
    // Wait until local object mapper is stopped
...
void System::SaveMapPointsOBJ(const string &filename)
{
    ...
    for(size_t i=0; i<vpMPs.size(); i++) {
        ...
        Eigen::Vector3f MPPositions = pMP->GetWorldPos();

        f << "v " << setprecision(7) << " " << MPPositions(0)
                                     << " " << MPPositions(1)
                                     << " " << MPPositions(2) << endl;
    }
...
{
    Map* pCurrentMap = mpAtlas->GetCurrentMap();
    auto objects = pCurrentMap->GetAllMapObjects();
    int i = 0;
    for (auto o : objects)
    {
