...
// Helper: convert Sophus::SE3f pose to Matrix34d (double-precision [R|t] matrix)
static Matrix34d SE3fToMatrix34d(const Sophus::SE3f& pose) {
    Eigen::Matrix4f Tcw_f = pose.matrix();
    Matrix34d Rt;
    Rt = Tcw_f.block<3, 4>(0, 0).cast<double>();
    return Rt;
}
...
bool ObjectTrack::ReconstructFromLandmarks(Map* map)
{
    ...
    for(size_t j = 0; j < points.size(); j++) {
            // ORB-SLAM3: GetWorldPos() returns Eigen::Vector3f directly
            Eigen::Vector3f p = points[j]->GetWorldPos();
            pts(0, j) = static_cast<double>(p(0));
            pts(1, j) = static_cast<double>(p(1));
            pts(2, j) = static_cast<double>(p(2));
    }
...
ObjectTrack::CopyDetectionsInKeyFrames()
{
    ...
    if (map_object_) {
        ...
        for (auto& it : keyframes_bboxes_) {
            ...
            // ORB-SLAM3: GetPose() returns Sophus::SE3f, convert to Matrix34d
            poses.push_back(SE3fToMatrix34d(it.first->GetPose()));
...
void ObjectTrack::TryResetEllipsoidFromMaPoints()
{
    ...
    if (degenerated_ellipsoid_ > 10) {
        ...
        Eigen::Vector3d s = 0.5 * solver.eigenvalues().cwiseAbs();
        if (R.determinant() < 0) {
            R.col(2) *= -1;
        }
...
void ObjectTrack::AssociatePointsInsideEllipsoid(Map* map)
{
    ...
    {
        // ORB-SLAM3: GetWorldPos() returns Eigen::Vector3f directly
        Eigen::Vector3f pos_f = p->GetWorldPos();
        Eigen::Vector3d pt = pos_f.cast<double>();
        if (ell.IsInside(pt)) {
