...
{
    local_object_mapper_ = pLocalObjectMapper;
}
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        // Relocalization (with OA-SLAM mode dispatch)
                        if (mpSystem->GetRelocalizationMode() == RELOC_POINTS) {
                            bOK = Relocalization();
                        } else if (mpSystem->GetRelocalizationMode() == RELOC_OBJECTS) {
                            bOK = RelocalizationFromObjects(false);
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            /////////////////////////////////// Objects Tracking ///////////////////////////////////
            // Update mean depth
            if (mState == Tracking::OK && mCurrentFrame.isSet()) {
                Eigen::Matrix4f Tcw_mat = mCurrentFrame.GetPose().matrix();
                Matrix34d Rt = Tcw_mat.cast<double>().topRows<3>();
                Eigen::Matrix3d K_eigen = mK_.cast<double>();
                double z_mean = 0.0;
                unsigned int z_nb = 0;
                for(size_t i = 0; i < mCurrentFrame.mvpMapPoints.size(); i++) {
                    MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
                    if(pMP && !mCurrentFrame.mvbOutlier[i]) {
                        Eigen::Vector3f mp = pMP->GetWorldPos();
                        Eigen::Vector4d p(mp(0), mp(1), mp(2), 1.0);
                ...
                if (current_frame_good_detections_.size() != 0) {
                    ...
                    for (size_t i = 0; i < current_frame_good_detections_.size(); ++i) {
                        ...
                        for (size_t j = 0; j < possible_tracks.size(); ++j) {
                            ...
                            if (det_cat != static_cast<int>(possible_tracks[j]->GetCategoryId()))
                                n = 0;
                            nb_matched_points[i].push_back(n);
                        }

                        if (max_nb_matches > static_cast<size_t>(THRESHOLD_NB_MATCH) &&
                    ...
                    if (!mbOnlyTracking) {
                        for (auto& tr : objectTracks_) {
                            ...
                            if (tr->GetNbObservations() >= 40 && tr->GetStatus() == ObjectTrackStatus::INITIALIZED && pCurMap) {
                                tr->OptimizeReconstruction(pCurMap);
                                auto checked = tr->CheckReprojectionIoU(0.3);
                                if (checked) {
                                    tr->InsertInMap(pCurMap);
                                    if (local_object_mapper_)
...
bool Tracking::RelocalizationFromObjects(bool use_points)
{
    ...
    if (!pCurMap)
        return false;
    auto objects = pCurMap->GetAllMapObjects();
    ...
    auto [best_index, poses, scores, used_pairs, used_inliers]
        = ORB_SLAM3::solveP3P_ransac(ellipsoids, ellipsoids_categories, bboxes, bboxes_categories, K_eigen, 0.2);
    ...
    for (size_t i = 0; i < indices.size() && scores[indices[i]] < bboxes.size(); ++i) {
        int cur_index = indices[i];

        const auto& pose_est = poses[cur_index];
        ...
        // Convert to Sophus::SE3f for ORB-SLAM3
        Eigen::Matrix4f Tcw_mat = Eigen::Matrix4f::Identity();
        for (int r = 0; r < 3; ++r)
            for (int c = 0; c < 4; ++c)
                Tcw_mat(r, c) = static_cast<float>(Rt_est(r, c));
        Sophus::SE3f Tcw_sophus(Tcw_mat);
        mCurrentFrame.SetPose(Tcw_sophus);

        // Also create cv::Mat for OptimizePoseFromObjects
        cv::Mat Rt(4, 4, CV_32F, 0.0f);
        for (int r = 0; r < 3; ++r)
            for (int c = 0; c < 4; ++c)
                Rt.at<float>(r, c) = static_cast<float>(Rt_est(r, c));
        Rt.at<float>(3, 3) = 1.0f;
        ...
        for (auto x : used_inliers[cur_index]) {
        ...
        Rt = OptimizePoseFromObjects(ellipses_inliers, ellipsoids_inliers, Rt, K_eigen);

        // Convert optimized cv::Mat pose back to Sophus::SE3f
        Eigen::Matrix4f opt_mat = Eigen::Matrix4f::Identity();
        for (int r = 0; r < 4; ++r)
            for (int c = 0; c < 4; ++c)
                opt_mat(r, c) = Rt.at<float>(r, c);
        Sophus::SE3f opt_pose(opt_mat);
        mCurrentFrame.SetPose(opt_pose);

        if (i == 0) {
            best_Rt_objects = Rt.clone();
        }

        // Match keypoints from the current frame to map points using the estimated pose
        auto map_points = pCurMap->GetAllMapPoints();
        ORBmatcher matcher(0.9, true);
    ...
    // Fall back to best object-only pose
    if (!best_Rt_objects.empty()) {
        Eigen::Matrix4f fallback_mat = Eigen::Matrix4f::Identity();
        for (int r = 0; r < 4; ++r)
            for (int c = 0; c < 4; ++c)
                fallback_mat(r, c) = best_Rt_objects.at<float>(r, c);
        Sophus::SE3f fallback_pose(fallback_mat);
        mCurrentFrame.SetPose(fallback_pose);
        mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.GetPose());
    }
